...
void nanite(const std::vector<Vertex>& vertices, const std::vector<unsigned int>& indices, std::vector<Cluster>& clusters, std::vector<clusterInfo>& clusterSizes)
{
	static const char* metis = getenv("METIS");
	if (metis && atoi(metis))
	{
	...
	clusters = clusterize(vertices, indices);
	...
	{
		clusterSize += clusters[pending[i]].indices.size();
	}

	clusterSizes.push_back(clusterInfo(clusters.size(), clusterSize, 0));
	unsigned int clusterOffset = clusters.size();
	...
	while (pending.size() > 1)
	{
		...
		for (size_t i = 0; i < groups.size(); ++i)
		{
			...
			{
				//split[j].self = groupb;
				split[j].self = bounds(vertices, split[j].indices, 0.f);
		...
		clusterSizes.push_back(clusterInfo(pending.size(), clusterSize, clusterOffset));
		clusterOffset += pending.size();
	...
	//TODO : implement multiple LOD on here
